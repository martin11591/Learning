...
:pageLink
set pageLink=!line!
if !requestsCount! GEQ %maxRequests% goto pageLink
...
set /a pageCount=!pageCount!+1
set /a pagesCount=!pagesCount!+1
set /a requestsCount=!requestsCount!+1
if EXIST "!file!" (
...
if NOT EXIST "!file!" (
  %wget% -O "!file!" "!pageLink!" --tries=0 --retry-connrefused --user-agent="Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/51.0.2704.84 Safari/537.36" -q >nul
)
:downloading
if EXIST "!file!" (
  for /f "tokens=*" %%b in ("!file!") do (
